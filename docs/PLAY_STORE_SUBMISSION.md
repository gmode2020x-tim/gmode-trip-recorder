...
- [x] Version name/code: `2.1.1` / `20101`
...
- [ ] Upload `GMODE-Trip-Recorder-v2.1.1-play.aab` to an internal test track first
...
3. Production: use staged rollout, monitor Android vitals, and retain v2.1.1 artifacts/checksums.
...
Adds live foreground GPS, accuracy, altitude, speed, course, coordinates, and GNSS satellite monitoring whenever the cockpit is visible, without saving trip points. Also includes authenticated Home Assistant phone-health logs, periodic diagnostics, revisioned notices and update metadata, bounded automatic-recording settings, acknowledged sync/re-arm commands, hybrid automatic recording, four exports, and offline HA catch-up.
