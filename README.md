...
- **Recommended phone package:** [GMODE-Trip-Recorder-v2.1.9-install.zip](https://github.com/gmode2020x-tim/gmode-trip-recorder/releases/download/v2.1.9/GMODE-Trip-Recorder-v2.1.9-install.zip)
- **APK only:** [GMODE-Trip-Recorder-v2.1.9-sideload.apk](https://github.com/gmode2020x-tim/gmode-trip-recorder/releases/download/v2.1.9/GMODE-Trip-Recorder-v2.1.9-sideload.apk)
- **Checksums:** [GMODE-Trip-Recorder-v2.1.9-SHA256SUMS.txt](https://github.com/gmode2020x-tim/gmode-trip-recorder/releases/download/v2.1.9/GMODE-Trip-Recorder-v2.1.9-SHA256SUMS.txt)
- **Release page:** [GMODE Trip Recorder v2.1.9](https://github.com/gmode2020x-tim/gmode-trip-recorder/releases/tag/v2.1.9)
...
- Restart-safe automatic trip pausing: after the configured stationary delay GMODE stops high-rate telemetry, watches at low power, and resumes the same trip only after precise GPS plus 30 seconds of sustained movement.
...
- Eleven selectable gauges with logical, sensor-specific scales and unlimited arrow navigation.
...
- `GMODE-Trip-Recorder-v2.1.9-sideload.apk` is signed with the existing sideload/debug lineage so it can update the previous GitHub APK without erasing local trips or settings.
- `GMODE-Trip-Recorder-v2.1.9-play.aab` is signed with the private upload key and is intended only for Google Play Console. Google Play App Signing supplies the distribution signature. Do not try to open the AAB on the phone.

### 1. Download and extract

1. Open the **recommended phone package** link above in Chrome or Samsung Internet.
2. If GitHub shows the release page, expand **Assets** and tap `GMODE-Trip-Recorder-v2.1.9-install.zip`.
3. When the download finishes, open Samsung **My Files > Downloads**.
4. Tap the ZIP, choose **Extract**, then open the extracted folder.
5. Tap `GMODE-Trip-Recorder-v2.1.9-sideload.apk`.
...
If Android reports that the package conflicts with an existing app, the installed copy was signed by a different key. Export any required trips, uninstall that incompatible copy, and then install v2.1.9.
...
```powershell
Get-FileHash .\GMODE-Trip-Recorder-v2.1.9-sideload.apk -Algorithm SHA256
```
